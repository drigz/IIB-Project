...
function from sampled inputs and outputs. Thus, when the unicycle runs, we get
a series of states and control inputs that are sampled from
$\boldsymbol{f}$, which can be used to estimate how the state will change
given any current state and control input. Crucially, since the output of GPR
is a Gaussian distribution, this method indicates the level of confidence its
predictions.
...
\begin{figure}[htbp]
  \begin{center}
    \includegraphics[width=14cm]{figures/GPR MLC.pdf}
    \end{center}

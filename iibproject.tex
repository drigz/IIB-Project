...
growing instability. (The blue lines show the prediction: the mean is dashed
and the 95\% confidence bounds are solid. The red line shows the trial
...
\begin{figure}[htpb]
  \begin{center}
    \subfigure[50ms timestep]{
      \includegraphics[height=6cm]{2d_50ms_angle.pdf}
      \label{fig:2d_50ms}
    }
    \subfigure[2$^\textrm{nd}$ order Markov]{
      \includegraphics[height=6cm]{2d_50ms_angle_history.pdf}
      \label{fig:2d_hist}
    }
    \end{center}
...
satisfactory and moved on to the 3D unicycle.
...
As a sidenote: there is a technique for dealing with motor dead-zones known as
``dithering'', which involves adding a high-frequency periodic signal to the
motor command, which will push the motor in and out of its dead-zone. The
system will filter this rapidly oscillating force out, effectively smoothing
the dead-zone out. McHutchon observed much better performing controllers for
the 2D system---ironically, it is possible that the high noise levels of his
angle sensors had a dithering effect on the motor, improving the performance.
...
\begin{figure}[htpb]
  \begin{center}
    \subfigure[Unicycle angles]{
      \includegraphics[width=12cm]{3d_1st_sequence_angle.pdf}
      \label{fig:3d_1st_sequence_angle}
    }
    \subfigure[Unicycle position]{
      \includegraphics[width=12cm]{3d_1st_sequence_dist.pdf}
...
(succeeded but performance was lacking / achieved short periods of balance)

(accuracy / confidence of final dynamic model)

Once / Now that the unicycle is capable of 

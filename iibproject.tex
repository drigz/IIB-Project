...
state distribution described in
Section~\ref{sec:initial_state_distribution}. After fixing this problem, the
simulations represented reality much better, and the RMLC system was able to
train better controllers. After 3 more trials, the trained controller could
keep the unicycle upright for the optimisation horizon of 5 seconds, as shown
in Figure~\ref{fig:3d_3rd_trial_17}. However, it is clear that the dynamics
model is not fully capturing the behaviour of the unicycle: the real trial's
trajectory makes large excursions outside of the predicted confidence
intervals. By this point, performing a single simulation took more than 20
minutes---typically, several hundred simulations are required to train an
optimal controller.

\begin{figure}[htpb]
  \begin{center}
    \includegraphics[height=8cm]{3d_3rd_trial_17.pdf}
    \end{center}
    \caption{Prediction and trial for the 17$^\textrm{th}$ controller of the
...
A major part of this project was the work on the unicycle's hardware and
software. Some of these tasks, such as building a controller and sensor for
the flywheel, were known from the start, whereas others, such as preventing
the toothed belt from slipping, were unexpected. However, by the end of the
project, the unicycle worked reasonably reliably, and the limited success of
the RMLC-trained controllers demonstrates that our efforts to develop a
controllable system were successful: the unicycle now has all sensors and
actuators necessary for balancing.

That said, the hardware leaves something to be desired. In particular, the
flywheel speed sensing is very noisy and the gear trains for the flywheel and
wheel both have sticking points, where the gears jam and the wheel is
considerably harder to turn. Despite these problems, and others, the 
...
\begin{itemize}
  ...
  \item When those conditions are satisfied, RMLC does a very good job of
    ...
    expert knowledge.
  \item Advantages of RMLC over conventional control.
    \end{itemize}

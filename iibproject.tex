...
the hardware and software of the unicycle in order to enable it to learn to
...
In many cases, these choices were made after realising deficiencies in the
...
\begin{figure}[htbp]
  \begin{center}
    ...
    \end{center}
    \caption{Pitch angle predictions with 1$^\textrm{st}$ and $^\textrm{nd}$
...
When we switched to a 2$^\textrm{nd}$ order Markov model, the initial
...
Figure~\ref{fig:con1_and_con2} shows the effect of this change.
...
had less of a change to try a variety of strategies, and necessarily is less
...
comparing dynamics model with similar numbers of points
...
system to allow balancing the unicycle, yet is extremely uncertain about its
predictions.
...
the trained controller could keep the unicycle upright for the optimisation
horizon of 5 seconds, as shown in Figure~\ref{fig:3d_3rd_trial_17}. However,
it is clear that the dynamics model is not fully capturing the behaviour of
the unicycle: the real trial's trajectory makes large excursions outside of
the predicted confidence intervals. Furthermore, trials will consistently
follow this unpredicted trajectory, as shown in
Figure~\ref{fig:3d_3rd_many_17}, which shows the trajectories of 9 trials
with the same controller. It also shows that the 5 second trial was
unrepresentative of the performance of the controller.
...
The smooth balance, achieved in the early stages of the 1$^\textrm{st}$ and
3$^\textrm{rd}$ sequences, was very promising. It showed that the system was
capable of balancing the unicycle, while also being able to track its motion
in the plane, as suggested by Figure~\ref{fig:3d_1st_sequence}. This would
suggest that it would be reasonably simple for the system to generate the
correct turning commands to follow a circular path around the target.
...
\subsubsection{Changed in the Dynamics}
...
learn the dynamics with extremely small amounts of experience with the system,
in most cases producing a reasonable controller with less that 10 seconds of
experience.
...
3$^\textrm{rd}$ 3D training sequence used 10 days of CPU-time on a high-end
